...
public class ProgressTracker : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (sfxSource && taskCompleteClip){
            sfxSource.spatialBlend = 0f; // 2D so you always hear it
            sfxSource.PlayOneShot(taskCompleteClip);
        }
            else
            {
                Debug.LogWarning("ProgressTracker: Missing sfxSource or taskCompleteClip.");
            }
        }
